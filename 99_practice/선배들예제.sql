...
SELECT NAME,PNAME, SUM(QUANTITY*PRICE)
FROM TBL_CUSTOM
JOIN TBL_BUY ON (CUSTOM_ID = CUSTOMID)
JOIN TBL_PRODUCT USING(PCODE)
WHERE PNAME LIKE '%사과%'
GROUP BY PCODE ,PNAME ,NAME;

--A-5. 총 구매합산 금액이 100000~200000 값인 고객 ID를 조회하시오.(김태완)


SELECT CUSTOM_ID, SUM(QUANTITY*PRICE)
FROM TBL_CUSTOM
...
-- 나이가 가장 많은 고객
-- AGE=(SELECT MAX(AGE) FROM TBL_CUSTOM)

-- 상품을 구매한 횟수
SELECT COUNT(*)
FROM TBL_BUY
JOIN TBL_CUSTOM ON(CUSTOMID = CUSTOM_ID)
WHERE CUSTOM_ID = (SELECT CUSTOM_ID
                FROM TBL_CUSTOM
                WHERE AGE=(SELECT MAX(AGE) FROM TBL_CUSTOM));
...
FROM TBL_BUY
JOIN TBL_PRODUCT USING(PCODE)
WHERE TO_CHAR(BUY_DATE, 'YYYY') = '2023' AND  TO_NUMBER(TO_CHAR(BUY_DATE, 'MM'))BETWEEN 7 AND 12
GROUP BY CUSTOMID
ORDER BY 2 DESC;
